...
void ImGuiWrapper::Construct(VkRenderPass renderpass)
{
    ...
    {
        auto scrollCallback = scroll_callback_t(&ScrollCallback);
        RenderingContext::RegisterScrollCallback(scroll_callback_t(&ScrollCallback));
        RenderingContext::RegisterCharCallback(char_callback_t(&CharCallback));
        RenderingContext::RegisterMouseButtonCallback(mouse_button_callback_t(&MouseButtonCallback));
        RenderingContext::RegisterKeyboardKeyCallback(keyboard_key_callback_t(&KeyCallback));
...
{
    descriptorPool = std::make_unique<vpr::DescriptorPool>(device->vkHandle(), 1, 0);
