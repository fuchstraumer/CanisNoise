        ...
        void ImageWriter::WritePNG(const char * filename, int compression_level) {
            ...
            for (size_t y = 0; y < height; ++y) {
                for (size_t x = 0; x < width; ++x) {
                    ...
                    pixelData[idx + 0] = static_cast<uint8_t>(scaled_data[width * y + x]);
                    pixelData[idx + 1] = static_cast<uint8_t>(scaled_data[width * y + x]);
                    pixelData[idx + 2] = static_cast<uint8_t>(scaled_data[width * y + x]);
                    pixelData[idx + 3] = 255;
                }
            }
